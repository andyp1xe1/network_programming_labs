...
class ServerTester:
    ...
    def test_rate_limit(self, requests_per_second, duration_seconds):
        ...
        print("\n=== Rate Limiting Test ===")
        print(
            f"Making {requests_per_second} requests/second for {duration_seconds} seconds"
        )
    ...
    def test_sustained_load(
        self, requests_per_second, duration_seconds, max_workers=10
    ):
        """Test sustained load using thread pool to maintain request rate"""
        print("\n=== Sustained Load Test ===")
        print(
            f"Maintaining {requests_per_second} requests/second for {duration_seconds} seconds"
        )
    ...
    def _print_results(self, total_time):
        ...
        if self.stats["total"] > 0:
            ...
            print(
                f"Total throughput: {self.stats['total'] / total_time:.2f} requests/second"
            )


def main():
    parser = argparse.ArgumentParser(
        description="Test HTTP server performance and rate limiting"
    )
    parser.add_argument(
        "url", help="Server URL to test (e.g., http://localhost:8080/index.html)"
    )
    parser.add_argument(
        "--rps",
        type=int,
        default=10,
        help="Requests per second for rate limiting test (default: 10)",
    )
    parser.add_argument(
        "--duration",
        type=int,
        default=5,
        help="Duration in seconds for rate limiting test (default: 5)",
    )
    parser.add_argument(
        "--concurrent",
        type=int,
        default=20,
        help="Number of concurrent requests for load test (default: 20)",
    )
    parser.add_argument(
        "--workers", type=int, help="Max worker threads (default: auto)"
    )
    parser.add_argument(
        "--test",
        choices=["load", "rate", "sustained", "all"],
        default="all",
